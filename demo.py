...
from src.text_utils import TextEncoder, set_up_special_tokens, fix_malformed
             ...
             for d in range(len(dims)):
                 ...
                 if use_mem and size_mem != 0:
                    ...
                    if args.decoding == 'topk':
                       ...
                       if use_mem:
                          gen = beam_search(model, XMB, encoder, i_1,num_beams=args.beam,size_mem=size_mem,use_mem=use_mem,pad_token_id=encoder['<|PAD|>'], eos_token_ids=[encoder['<|endoftext|>']])
                       else:
                          gen = beam_search(model, XMB, encoder, i_1,num_beams=args.beam,pad_token_id=encoder['<|PAD|>'], eos_token_ids=[encoder['<|endoftext|>']])
